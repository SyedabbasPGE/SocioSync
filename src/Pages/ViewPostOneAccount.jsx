...
import { useLocation } from "react-router-dom"; // To get account information passed as state

const ViewPostOneAccount = () => {
  const [posts, setPosts] = useState([]);
  const [accountName, setAccountName] = useState("");

  // Get account name from the location state
  const location = useLocation();
  useEffect(() => {
    if (location.state && location.state.user && location.state.user.username) {
      setAccountName(location.state.user.username); // Set the account name
    }
  }, [location]);
  ...
  // Filter posts by the specific account and valid data
  const filteredPosts = posts.filter(
    (post) =>
      Array.isArray(post.accounts) &&
      post.accounts.includes(accountName) && // Show posts for the specific account
      post.postText &&
      post.postText.trim() !== "" &&
      post.postTime &&
      post.postTime.trim() !== ""
  ...
  return (
    <div className="p-8 bg-gradient-to-br from-gray-800 via-gray-700 to-slate-900 min-h-screen">
      <h1 className="text-xl font-bold text-white mb-4">
        Posts for Account: <span className="text-yellow-400">{accountName}</span>
      </h1>
      {filteredPosts.length > 0 ? (
        ...
        <p className="text-gray-400 text-center mt-8">No posts found for this account.</p>
      )}
    </div>
  );
};

export default ViewPostOneAccount;
